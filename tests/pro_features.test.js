...
function candles(n=180){ const end=Date.now()-15*60*1000; return Array.from({length:n},(_,i)=>({time:end-(n-1-i)*900000,open:100+i*.05,high:101+i*.05,low:99+i*.05,close:100+i*.05,volume:1000+(i%10)*50})); }
const c=candles();
...
// KuCoin timestamp normalization regression: legacy Unix seconds remain
// accepted by the validator and are normalized to internal milliseconds.
{
  const assert = require('assert');
  const { DataValidator } = require('../data-validator');
  const now = Date.now();
  const validator = new DataValidator({ maxAgeMs: 30 * 60 * 1000 });
  const sec = Math.floor((now - 2 * 60 * 1000) / 1000);
  const legacySeconds = Array.from({ length: 20 }, (_, i) => ({
