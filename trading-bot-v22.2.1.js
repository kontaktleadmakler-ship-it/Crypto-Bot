/**
 * ============================================================================
 * TRADING SIGNAL BOT - v22.2.1 PRO DYNAMIC VOLATILITY SURFACE & DQN EDITION
...
async function sendDeduplicatedAlert(key, text, cooldownMs = 300000) {
  ...
  const now = Date.now();
  try {
    const result = await botStateCollection.findOneAndUpdate(
      { _id: `alert_${key}`, $or: [{ lastSent: { $exists: false } }, { lastSent: { $lte: now - cooldownMs } }] },
      { $set: { lastSent: now } },
    ...
    logger.warn(`[ALERT-COOLDOWN] ${key}: ${e.message}`);
  }
}
...
const config = {
  ...
  DATA_MAX_GAP_FACTOR_15M: parseFloat(process.env.DATA_MAX_GAP_FACTOR_15M) || 4,
  DATA_MAX_GAP_RATIO_15M: parseFloat(process.env.DATA_MAX_GAP_RATIO_15M) || 0.25,
  ENABLE_ADVANCED_MTF: process.env.ENABLE_ADVANCED_MTF === 'true',
  ADVANCED_MTF_MAX_SYMBOLS: parseInt(process.env.ADVANCED_MTF_MAX_SYMBOLS, 10) || 20,
  CVD_ENABLED: process.env.CVD_ENABLED !== 'false',
  CVD_LOOKBACK: parseInt(process.env.CVD_LOOKBACK, 10) || 20,
  VWMACD_FAST: parseInt(process.env.VWMACD_FAST, 10) || 12,
  VWMACD_SLOW: parseInt(process.env.VWMACD_SLOW, 10) || 26,
  VWMACD_SIGNAL: parseInt(process.env.VWMACD_SIGNAL, 10) || 9,
  ICHIMOKU_CONVERSION: parseInt(process.env.ICHIMOKU_CONVERSION, 10) || 9,
  ICHIMOKU_BASE: parseInt(process.env.ICHIMOKU_BASE, 10) || 26,
  ICHIMOKU_SPAN_B: parseInt(process.env.ICHIMOKU_SPAN_B, 10) || 52,
  FIB_LOOKBACK: parseInt(process.env.FIB_LOOKBACK, 10) || 120,
  FIB_ZONE_TOLERANCE: parseFloat(process.env.FIB_ZONE_TOLERANCE) || 0.004,
  EXECUTION_ENABLED: process.env.EXECUTION_ENABLED === 'true',
  DRY_RUN: process.env.DRY_RUN !== 'false',
  KILL_SWITCH_ENABLED: process.env.KILL_SWITCH_ENABLED !== 'false',
...
function validateConfig() {
  ...
  logger.info(`🛡️ Phase-A Hardening aktiv | DataValidation + CentralRisk + AuditTrail + OOS/MonteCarlo`);
}
validateConfig();
...
const executionAdapter = new KuCoinFuturesExecution({ apiKey: config.KUCOIN_API_KEY, apiSecret: config.KUCOIN_API_SECRET, apiPassphrase: config.KUCOIN_API_PASSPHRASE, dryRun: config.DRY_RUN, killSwitch: config.KILL_SWITCH_ENABLED, logger });
const cvdStreams = new Map();
...
function buildDQNStateVector(params) {
  return [
    ...
    params.confluenceScore != null ? Number(params.confluenceScore) / 100 : 0.5
  ];
}
...
async function persistDqnState() {
  if (!botStateCollection || !isDbConnected) return;
  await botStateCollection.updateOne({ _id: 'botControl' }, { $set: { dqnEnabled: !!config.DQN_ENABLED } }, { upsert: true });
}

async function setDqnEnabled(enabled) {
  config.DQN_ENABLED = !!enabled;
  await persistDqnState();
  if (config.DQN_ENABLED) {
    await dqnAgent.init();
    await auditLogger.write('DQN_ENABLED', { enabled: true, modelVersion: dqnAgent.modelVersion });
  } else {
    await auditLogger.write('DQN_DISABLED', { enabled: false });
  }
}
...
async function initDatabase() {
  try {
    ...
    auditLogger.collection = db.collection('auditLogs');
    isDbConnected = true;
    apiLatencyStats.record('mongodb', Date.now() - startTime);
    ...
    try {
      ...
      await auditLogger.collection.createIndex({ timestamp: -1 });
    } catch (e) {}
    ...
    const dqnControl = await botStateCollection.findOne({ _id: 'botControl' });
    if (dqnControl && typeof dqnControl.dqnEnabled === 'boolean') config.DQN_ENABLED = dqnControl.dqnEnabled;
...
function calculateSignalScore(params) {
  ...
  const advanced = [params.advancedConfluence, params.ichimokuScore, params.volumeMACDScore, params.cvdScore].filter(Number.isFinite);
  if (advanced.length) { const advancedScore = advanced.reduce((a,b)=>a+b,0)/advanced.length; score = score * 0.70 + advancedScore * 0.30; }
...
async function fetchKucoinKlines(symbol, timeframe = '15m', limit = 100) {
  ...
  // KuCoin Futures currently expects `from`/`to` in Unix milliseconds
  // on GET /api/v1/kline/query. Normalize returned candle timestamps to
  // milliseconds internally, while tolerating legacy second-based payloads.
  const nowMs = Date.now();
  const to = nowMs;
  const from = to - Math.ceil((limit + 10) * granularity * 60 * 1000);
  ...
  try {
    ...
    if (res.data && res.data.code === '200000' && Array.isArray(res.data.data)) {
      ...
      const candles = res.data.data.map(c => {
        const rawTime = parseInt(c[0], 10);
        const time = rawTime > 0 && rawTime < 1e12 ? rawTime * 1000 : rawTime;
...
function formatScanStatsReport(stats) {
  const lines = [`🔎 <b>SCAN-DIAGNOSE v22.2.1 (${escapeHtml(STRATEGY_PROFILE_NAME)})</b>`];
...
async function scanMarket() {
  ...
  logger.info(`[${new Date().toISOString().slice(0, 16)}] 🔍 Starte Scan v22.2.1 (mit DQN)...`);
  ...
  try {
    ...
    const btcVolatility = btcPrice > 0 ? btcATR / btcPrice : 0;
    ...
    const spotWatchlist = await getTopKucoinPairs(config.TOP_COIN_LIMIT).catch((e) => {
      logger.error(`🔴 [DATA-QUALITY] Watchlist konnte nicht geladen werden: ${e.message}`);
      return [];
    });
    ...
    await asyncPool(config.SCAN_CONCURRENCY, dynamicWatchlist, async (symbol) => {
      ...
      try {
        ...
        const dataCheck = raw15m ? dataValidator.candles(raw15m, {
          ...
          minLength: 20,
          maxAgeMs: config.DATA_MAX_AGE_15M_MS,
          allowGaps: true,
        ...
        if (dataCheck.warnings?.length) {
          const w = dataCheck.warnings[0];
          logger.debug?.(`ℹ️ [DATA-QUALITY] ${symbol}/15m: ${w.gapCount} fehlende Intervalle, maxGap=${w.maxGapIntervals}`);
        }
        ...
        let cvdTrades = [];
        if (config.CVD_ENABLED && cvdStreams.size < config.ADVANCED_MTF_MAX_SYMBOLS && !cvdStreams.has(symbol)) {
          try { const stream = await new KuCoinTradeStream({ symbol: getFuturesSymbol(symbol), logger }).start(); cvdStreams.set(symbol, stream); } catch (e) { logger.warn(`[CVD] ${symbol}: ${e.message}`); }
        }
        ...
        if (direction !== null) {
          ...
          const advancedFeatures = buildFeatureSnapshot({ candlesByTf: { '1m': [], '5m': [], '15m': raw15m, '1h': raw1h, '4h': raw4h || [] }, phase: currentMarketPhase, direction, cvdTrades, config });
          const signalScore = calculateSignalScore({
            ...
            advancedConfluence: advancedFeatures.confluence.score,
            ichimokuScore: advancedFeatures.ichimokuScore,
            volumeMACDScore: advancedFeatures.volumeMACDScore,
            cvdScore: advancedFeatures.cvdScore
          });
          ...
          const finalCheck = riskEngine.evaluate({
            equityUSD: config.CAPITAL_USD + dailyNetPnL,
          ...
          if (!finalCheck.allowed) {
            const reason = `risk_${String(finalCheck.reason).replace(/[^a-z0-9_-]/gi, '_')}`;
            scanStats[reason] = (scanStats[reason] || 0) + 1;
            await auditLogger.write('SIGNAL_BLOCKED_RISK', { symbol, direction, reason: finalCheck.reason });
            return;
          }
...
async function handleTelegramCommand(chatId, text) {
  ...
  if (command === '/help' || command === '/start') {
    await sendTelegramReply(chatId,
      `<b>🤖 TRADING BOT v22.2.1 PRO - DQN & MODULAR SYSTEM</b>\n` +
      `━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n` +
      ...
      `/dqn [on|off|status] - DQN Paper-Learning/Veto steuern\n` +
  ...
  if (command === '/dqn') {
    ...
    if (!['on','off','status'].includes(action)) {
      await sendTelegramReply(chatId, '⚠️ Syntax: <code>/dqn on</code>, <code>/dqn off</code> oder <code>/dqn status</code>');
      return;
    }
    if (action === 'status') {
      const st = dqnAgent.getStats();
      await sendTelegramReply(chatId, `🤖 <b>DQN STATUS</b>\n• Aktiv: <b>${config.DQN_ENABLED ? '🟢 JA' : '🔴 NEIN'}</b>\n• Initialisiert: ${st.isInitialized ? 'Ja' : 'Nein'}\n• Epsilon: ${st.epsilon}\n• Memory: ${st.memorySize}\n• Modus: Paper Trading / Learning`);
      return;
    }
    try {
      await setDqnEnabled(action === 'on');
      await sendTelegramReply(chatId, action === 'on' ? '🟢 <b>DQN AKTIVIERT</b> – Paper-Learning/Veto ist eingeschaltet.' : '🔴 <b>DQN DEAKTIVIERT</b> – Bot läuft ohne DQN-Veto und ohne DQN-Training.');
    } catch (e) {
      await sendTelegramReply(chatId, `❌ DQN-Änderung fehlgeschlagen: ${escapeHtml(e.message)}`);
    }
    return;
  }
  ...
  if (command === '/optimize') {
    ...
    try {
      ...
      if (best) {
        await auditLogger.write('OPTIMIZATION_COMPLETED', { symbol: symbolArg, days, bestParams: best });
        ...
        report += `ℹ️ <i>Research-Ergebnis: Live-Parameter wurden NICHT verändert.</i>`;
  ...
  if (command === '/status') {
    const lines = [];
    lines.push(`🤖 <b>BOT STATUS v22.2.1 PRO DQN</b>`);
    lines.push(`━━━━━━━━━━━━━━━━━━━━━━━━`);
...
app.get('/', (req, res) => {
  res.send(`🤖 Trading Bot v22.2.1 PRO | Phase: ${currentMarketPhase} | DB: ${isDbConnected ? '✅' : '🔴'}`);
});
...
// ==========================================
(async () => {
  logger.info('🚀 Starte Trading Bot v22.2.1 PRO (Full Features, TensorFlow.js ML, DQN Agent, Cross-Hedging, Volatility Surface & Order Flow)...');
