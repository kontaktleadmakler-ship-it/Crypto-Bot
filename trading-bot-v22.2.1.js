...
async function fetchKucoinKlines(symbol, timeframe = '15m', limit = 100) {
  ...
  // KuCoin Futures Kline timestamps are Unix seconds in the current API.
  // Keep our internal representation in milliseconds for consistency.
  const nowMs = Date.now();
  const to = Math.floor(nowMs / 1000);
  const from = to - Math.ceil((limit + 10) * granularity * 60);
