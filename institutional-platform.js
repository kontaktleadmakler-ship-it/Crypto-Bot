...
class InstitutionalPlatform{constructor(opts={}){this.backtest=new InstitutionalBacktestEngine(opts.backtest);this.monteCarlo=new MonteCarloEngine(opts.monteCarlo);this.walkForward=new WalkForwardEngine(opts.walkForward);this.oos=new OOSValidator();this.ml=new MLEvaluationFramework();this.paperShadow=new InstitutionalPaperShadowSuite(opts.paperShadow);this.testnet=new BinanceTestnetAdapter({enabled:opts.testnetEnabled===true,mode:'SIMULATED'});this.metrics=new MetricsRegistry();this.health=new HealthMonitor({metrics:this.metrics});this.alerts=new AlertEngine(opts.alerts);this.dashboard=new Dashboard({health:this.health,metrics:this.metrics,alerts:this.alerts});this.reports=new ReportGenerator(opts.reporting)}
 ...
 status(){return {version:'24.6.0',mode:'SIGNAL_PAPER_SHADOW',liveOrderExecution:false,health:this.dashboard.snapshot(),paperShadow:this.paperShadow.snapshot(),testnet:this.testnet.snapshot()}}
}
