...
function calculateVWAP(candles) {
  ...
  const d=new Date(candles.at(-1).time), start=Date.UTC(d.getUTCFullYear(),d.getUTCMonth(),d.getUTCDate())/1000;
  const ws=candles.filter(c=>c.time>=start); let tpv=0,v=0;
...
function aggregate(candles, periods){const out=[];for(let i=periods-1;i<candles.length;i+=periods){const s=candles.slice(i-periods+1,i+1);out.push({time:s.at(-1).time,open:s[0].open,high:Math.max(...s.map(c=>c.high)),low:Math.min(...s.map(c=>c.low)),close:s.at(-1).close,volume:s.reduce((a,c)=>a+c.volume,0)});}return out;}
...
function evaluateGates(dir,p,cfg){const long=dir==='LONG';if(cfg.REQUIRE_4H_TREND&&p.trend4h!==(long?'BULLISH':'BEARISH'))return false;if(p.trend1h!==(long?'BULLISH':'BEARISH')||p.trend15m!==(long?'BULLISH':'BEARISH'))return false;if(!cfg.ALLOW_COUNTER_BTC_TREND&&((p.btcTrend==='BEARISH'&&long)||(p.btcTrend==='BULLISH'&&!long)))return false;if(p.adx<p.adaptive.adx||p.hurst<cfg.MIN_HURST_EXPONENT||p.chop>cfg.MAX_CHOP_INDEX)return false;if(long?!p.bosBullish:!p.bosBearish)return false;if(long?(p.rsi<cfg.RSI_LONG_MIN||p.rsi>cfg.RSI_LONG_MAX):(p.rsi<cfg.RSI_SHORT_MIN||p.rsi>cfg.RSI_SHORT_MAX))return false;if(long?(p.price<p.poc||p.price<p.vwap):(p.price>p.poc||p.price>p.vwap))return false;if(long?p.macd.histogram<0:p.macd.histogram>0)return false;if(long?p.relativeVolume<p.adaptive.vol:p.relativeVolume<p.adaptive.vol)return false;return true;}
...
async function trainModelFromRecords(model,records,cfg){if(!records||records.length<cfg.ML_MIN_TRAINING_SAMPLES)return false;const positives=records.filter(r=>r.pnlUSD>0).length;if(positives<5||records.length-positives<5)return false;
  const ds=records.slice(-cfg.BACKTEST_MAX_TRAIN_TRADES).map(r=>({f:model.featuresFromTrade(r),y:r.pnlUSD>0?1:0}));const split=Math.max(1,Math.floor(ds.length*.8));const tr=ds.slice(0,split),va=ds.slice(split);if(!va.length)return false;const scaler=model.makeScaler(tr.map(x=>x.f));const xs=tf.tensor2d(model.scaleMatrix(tr.map(x=>x.f),scaler));const ys=tf.tensor2d(tr.map(x=>[x.y]));const vx=tf.tensor2d(model.scaleMatrix(va.map(x=>x.f),scaler));const vy=tf.tensor2d(va.map(x=>[x.y]));const net=tf.sequential();net.add(tf.layers.dense({inputShape:[17],units:32,activation:'relu',kernelInitializer:'heNormal'}));net.add(tf.layers.dropout({rate:.15}));net.add(tf.layers.dense({units:16,activation:'relu'}));net.add(tf.layers.dense({units:8,activation:'relu'}));net.add(tf.layers.dense({units:1,activation:'sigmoid'}));net.compile({optimizer:tf.train.adam(.001),loss:'binaryCrossentropy',metrics:['accuracy']});await net.fit(xs,ys,{epochs:cfg.ML_EPOCHS,batchSize:Math.min(cfg.ML_BATCH_SIZE,tr.length),validationData:[vx,vy],shuffle:false,verbose:0,callbacks:tf.callbacks.earlyStopping({monitor:'val_loss',patience:7,restoreBestWeight:true})});[xs,ys,vx,vy].forEach(t=>t.dispose());if(model.model)model.model.dispose();model.model=net;model.scaler=scaler;model.trained=true;return true;}

function predictWith(model,features){return model.predict(features);}

function simulateSignal(signal, bars, startIndex, cfg){const entry=applySlippage(bars[startIndex].open,signal.direction,cfg.SLIPPAGE_PERCENT,'entry');const stopDist=signal.atr*signal.adaptive.atr;let stop=signal.direction==='LONG'?entry-stopDist:entry+stopDist;const tp1=signal.direction==='LONG'?entry+stopDist*signal.adaptive.tp1:entry-stopDist*signal.adaptive.tp1;const tp2=signal.direction==='LONG'?entry+stopDist*cfg.TP2_MULT:entry-stopDist*cfg.TP2_MULT;const riskPerUnit=Math.abs(entry-stop);const riskUSD=cfg.BACKTEST_STARTING_CAPITAL*(cfg.RISK_PERCENT/100);const units=riskPerUnit>0?riskUSD/riskPerUnit:0;let remaining=units,realized=0,tp1Hit=false,highest=entry,lowest=entry;const entryFee=fee(units*entry,cfg.FEE_PERCENT);let exitPrice=bars[startIndex].open,reason='end';let lastIndex=startIndex;
...
async function runBacktest({symbol='BTC-USDT',days=30,cfg=buildConfig(process.env),useML=cfg.BACKTEST_USE_ML,walkForward=true,logger=console}={}){
  logger.log(`📥 Lade ${symbol} 15m-Daten für ${days} Tage...`);const [bars,btc]=await Promise.all([fetchKucoinCandles(symbol,'15m',days),symbol==='BTC-USDT'?Promise.resolve([]):fetchKucoinCandles('BTC-USDT','15m',days)]);if(bars.length<cfg.BACKTEST_WARMUP_BARS+100)throw new Error(`Zu wenige 15m-Kerzen: ${bars.length}`);const btcBars=btc.length?btc:bars;
  ...
  for(let i=warm;i<bars.length-1;i++){
    ...
    const c15=bars.slice(0,i+1);const c1=tf1h.filter(c=>c.time<=bar.time);const c4=tf4h.filter(c=>c.time<=bar.time);const bc=btcBars.slice(0,Math.min(btcBars.length,Math.floor(i*btcBars.length/bars.length)+1));const snap=buildSnapshot(c15,c1,c4,bc,cfg);if(!snap.poc||!snap.vwap||!snap.atr)continue;const sig=candidate(snap,cfg);if(!sig)continue;rawCandidates++;
    const nextIndex=i+1;sig.entry=bars[nextIndex].open;signals++;
    let accepted=true,prob=.5;if(useML&&walkForward){if(trainingRecords.length>=cfg.ML_MIN_TRAINING_SAMPLES && (trainingRecords.length===cfg.ML_MIN_TRAINING_SAMPLES || signals%cfg.BACKTEST_RETRAIN_EVERY_SIGNALS===0)){if(await trainModelFromRecords(model,trainingRecords,cfg))mlRetrains++;}if(model.trained){const f=tradeFeatures(model,sig),pred=predictWith(model,f);prob=pred.probability;if(prob<cfg.ML_MIN_PREDICTION_PROBABILITY){accepted=false;mlBlocked++;}else mlAccepted++;}}
