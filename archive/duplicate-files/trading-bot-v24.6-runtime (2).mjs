...
const futuresApiSemaphore = {
  ...
  async acquire() {
    ...
    // FIX (2026-08-30): this wait used to be unbounded. If every active
    // slot is pinned in a 429-backoff retry loop (see getContract/
    // getOrderBook fix above), a queued waiter could sit here past the
    // 16s market-data bundle timeout with no error ever surfacing - the
    // bundle just silently ran out the clock. Bounding the wait lets a
    // saturated queue fail fast and visibly instead.
    const SEMAPHORE_WAIT_TIMEOUT_MS = 10000;
    let queueEntry;
    await new Promise((resolve, reject) => {
      ...
      setTimeout(() => {
        ...
        if (idx !== -1) {
          ...
          reject(new Error('FUTURES_API_SEMAPHORE_QUEUE_TIMEOUT'));
        }
...
// FIX (2026-08-30): the scan watchdog only ever reset `isScanning`, it
// never stopped the timed-out scan's already-running asyncPool loop -
// JS can't cancel an in-flight Promise chain from outside. So a forced
// watchdog kill left the old scan's candidate loop running as a
// zombie in the background while, since `isScanning` was already back
// to false, the next cron tick started a brand new scan over the same
// watchlist. Both then competed for the same 3 futuresApiSemaphore
// slots, guaranteeing the new scan ALSO missed its deadline, spawning
// another zombie, compounding without bound (visible as the same
// symbol being logged twice per tick). `scanGeneration` lets a scan's
// per-candidate loop recognize it has been superseded and stop issuing
// new requests, so it drains instead of piling up.
...
async function scanMarket() {
  ...
  const scanWatchdogTimer = setTimeout(() => {
    if (isScanning) {
      ...
      isScanning = false;
      // Supersede this scan's generation so its still-running asyncPool
      // loop (if any) stops launching new per-symbol work on its next
      // check, instead of continuing as an unbounded zombie alongside
      // whatever scan starts next.
      scanGeneration++;
  ...
  try {
    ...
    await asyncPool(config.SCAN_CONCURRENCY, dynamicWatchlist, async (symbol) => {
      // FIX (2026-08-30): bail out immediately if this scan has been
      // superseded (watchdog kill or a fresh scan elsewhere bumped the
      // generation) instead of continuing to fire off new requests as a
      // zombie loop competing with whatever scan is now active. See
      // scanGeneration comment above scanMarket().
      if (scanGeneration !== myScanGeneration) return;
