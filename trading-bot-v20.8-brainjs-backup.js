/**
 * ============================================================================
 * TRADING SIGNAL BOT - ULTIMATE v20.8 FULL COMPLETE EDITION (PATCHED)
 * (Mit adaptivem Brain.js ML, globaler Telegram-Queue, State-Persistenz & Hurst-Exponent)
 * ============================================================================
 */
...
// ==========================================
// 7. DATENBANK & ADAPTIVES ML-MODELL
// ==========================================
...
// Brain.js Neural Network für adaptives ML
const mlNet = new brain.NeuralNetwork({
  hiddenLayers: [4],
  activation: 'sigmoid'
});
let isModelTrained = false;

async function trainSignalMLModel() {
  try {
    if (!closedTradesCollection || !isDbConnected) return;
    const pastTrades = await closedTradesCollection.find({}).sort({ closeTime: -1 }).limit(50).toArray();
    if (pastTrades.length < 10) return;

    const recentChunk = pastTrades.slice(0, 10);
    const recentWins = recentChunk.filter(t => (t.pnlUSD || 0) > 0).length;
    const recentWinRate = (recentWins / recentChunk.length) * 100;

    let targetIterations = 2000;
    let targetError = 0.005;

    if (recentWinRate < 40) {
      targetIterations = 4000;
      targetError = 0.002;
      logger.warn(`🧠 [ML-Adaptive] Schlechte Phase erkannt (Win-Rate: ${recentWinRate.toFixed(1)}%). Erhöhe KI-Strenge!`);
    } else if (recentWinRate >= 70) {
      targetIterations = 1500;
      targetError = 0.008;
      logger.info(`🧠 [ML-Adaptive] Starke Phase erkannt (Win-Rate: ${recentWinRate.toFixed(1)}%). KI passt sich an.`);
    }

    const trainingData = pastTrades.map(t => ({
      input: [
        Math.min(t.adxAtEntry || 20, 100) / 100,
        Math.min(t.rsiAtEntry || 50, 100) / 100,
        Math.min(t.relativeVolumeAtEntry || 1, 5) / 5,
        (t.signalScore || 50) / 100
      ],
      output: [ (t.pnlUSD || 0) > 0 ? 1 : 0 ]
    }));

    mlNet.train(trainingData, { iterations: targetIterations, errorThresh: targetError, log: false });
    isModelTrained = true;
    logger.info(`🧠 [ML-Adaptive] KI-Modell trainiert (Iterationen: ${targetIterations}, Fehler-Limit: ${targetError}, Win-Rate: ${recentWinRate.toFixed(1)}%).`);
  } catch (e) {
    logger.error(`[ML-Adaptive Fehler beim Training]: ${e.message}`);
  }
}

function predictSignalSuccess(adx, rsi, relativeVolume, signalScore) {
  if (!isModelTrained) return 0.5;
  const result = mlNet.run([
    Math.min(adx, 100) / 100,
    Math.min(rsi, 100) / 100,
    Math.min(relativeVolume, 5) / 5,
    signalScore / 100
  ]);
  return result[0];
}
...
function formatScanStatsReport(stats) {
  const lines = [`🔎 <b>SCAN-DIAGNOSE v20.8 (${escapeHtml(STRATEGY_PROFILE_NAME)})</b>`];
  ...
  if (stats.mlBlocked) reasons.push(`Brain.js ML-Filter blockiert: ${stats.mlBlocked}`);
...
async function scanMarket() {
  ...
  logger.info(`[${new Date().toISOString().slice(0, 16)}] 🔍 Starte Scan v20.8...`);
    ...
    await asyncPool(config.SCAN_CONCURRENCY, dynamicWatchlist, async (symbol) => {
      ...
      try {
        ...
        if (direction) {
          ...
          const mlSuccessProbability = predictSignalSuccess(adx, rsi, relativeVolume, signalScore);
          if (isModelTrained && mlSuccessProbability < 0.40) {
            scanStats.mlBlocked++;
            return;
          }
          ...
          if (config.ENABLE_ORDERBOOK_ANALYSIS && signalScore > 60) {
            const obRatio = await fetchOrderBookImbalance(symbol).catch(() => null);
            if (obRatio !== null) {
              const obOk = direction === 'LONG' ? obRatio > 0.9 : obRatio < 1.1;
              if (!obOk) {
...
async function handleTelegramCommand(chatId, text) {
  ...
  if (command === '/help' || command === '/start') {
    await sendTelegramReply(chatId,
      `<b>🤖 TRADING BOT v20.8 ULTIMATE - BEFEHLE</b>\n` +
      `━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n` +
  ...
  if (command === '/status') {
    const lines = [];
    lines.push(`🤖 <b>BOT STATUS v20.8 ULTIMATE</b>`);
    lines.push(`━━━━━━━━━━━━━━━━━━━━━━━━`);
...
app.get('/', (req, res) => {
  res.send(`🤖 Trading Bot v20.8 ULTIMATE | Phase: ${currentMarketPhase} | DB: ${isDbConnected ? '✅' : '🔴'}`);
});
...
app.get('/health', (req, res) => {
  ...
  res.status(isDbConnected ? 200 : 503).json({
    status: isDbConnected ? 'ok' : 'degraded', version: '20.8', dbConnected: isDbConnected,
...
// ==========================================
(async () => {
  logger.info('🚀 Starte Trading Bot v20.8 ULTIMATE (Full Features, Brain.js ML & Hurst Filter)...');
  await initDatabase();
  await loadFuturesContractSpecs();
  await trainSignalMLModel();
  pollTelegramUpdates();
