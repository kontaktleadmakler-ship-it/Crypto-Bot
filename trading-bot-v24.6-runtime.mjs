...
const client = new MongoClient(config.MONGODB_URI, {
  ...
  // FIX (2026-08-30): socketTimeoutMS only bounds an ALREADY-ACQUIRED socket
  // that goes idle mid-operation. It does nothing for an operation still
  // queued waiting for a free connection out of the pool - without
  // waitQueueTimeoutMS that wait is unbounded. Under concurrent load (scan
  // pool + bulk queue + alert history + execution event store all sharing
  // this one client) a handful of slow/pinned connections can exhaust
  // maxPoolSize and leave every subsequent await stuck in the driver's
  // internal wait queue with no error and no log line, indistinguishable
  // from a genuine hang - the only thing that ever unstuck it was asyncPool's
  // outer 75s per-item watchdog, well past the 16-20s market-data timeout
  // that's supposed to catch slow calls first. Bounding the wait queue makes
  // pool exhaustion fail fast and visibly instead of silently eating the
  // whole per-item budget.
